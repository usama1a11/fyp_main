...
class PendingOrdersScreen extends StatelessWidget {
  ...
  Future<void> moveToOnTheWay(String? orderKey) async {
    if (orderKey != null) {
      await _dbRef.child(orderKey).update({
        'status': 'On The Way',
      });
...
class CompletedOrdersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: AllOrdersScreen().buildOrdersListView('Completed'),
    );
  }

  // This method is called when the user presses the cancel button
  Future<void> cancelOrder(BuildContext context, String orderKey) async {
    if (orderKey != null) {
      try {
        // Update the status of the order to 'Canceled' instead of removing it
...
/*class CompletedOrdersScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return PendingOrdersScreen().buildOrdersListView('Completed');
  }
}*/
// Implement onthewaysScreen
//real
/*class OnTheWayOrdersScreen extends StatelessWidget {
  final DatabaseReference _dbRef = FirebaseDatabase.instance.ref().child('orders');
  ...
  @override
  Widget build(BuildContext context) {
    return AllOrdersScreen().buildOrdersListView('On The Way');
  }
}*/
// Separate screen for Pending Orders
// final pending
/*class PendingOrdersScreen extends StatelessWidget {
  final DatabaseReference _dbRef = FirebaseDatabase.instance.ref().child('orders');
      ...
      child: Stack(
          children: [
            ...
            Stack(
              children: [
                Padding(
                  padding: const EdgeInsets.only(left: 275.0,top: 8),
                  child: IconButton(
                    onPressed: (){
                      cancelOrder(order['key']);
                    },
